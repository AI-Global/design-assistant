...
test('Signup fails email validation', async () => {
  ...
  let errors = {
    response: {
      data: { email: { isInvalid: true, message: 'email already exists' } },
    },
  };
...
test('Signup fails username validation', async () => {
  ...
  let errors = {
    response: {
      data: { password: { isInvalid: true, message: 'weak password' } },
    },
  };
  axios.post.mockRejectedValue(errors);
  fireEvent.click(screen.getByText('Create My Account'));
...
test('Signup fails confirm password validation', async () => {
  ...
  await wait(() =>
    expect(
      screen.getByText("Those passwords didn't match. Please try again.")
    ).toBeTruthy()
  );
});
