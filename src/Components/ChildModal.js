...
export default function ChildModal(props) {
  ...
  var currentQuestion = props.current_question
    ...
    : '0';
  var previousQuestion = props.previous_question
    ? props.previous_question.question
    : '0';
  var responses =
    props.previous_question && props.previous_question
      ? props.previous_question.responses
      : [];
  ...
  function save() {
    ...
    if (makeChild) {
      ...
      props.current_question.trigger = {
        parent: parentId,
        responses: triggers,
    ...
    >
      ...
      <Modal.Body>
        <div className="form-label">
          Would you like to form this question relationship:
        </div>
        <Row>
          <Col md={12}>
        ...
        <Form>
          ...
          {!makeChild ? null : (
            <Form.Group>
              <div className="form-label">
                Which response(s) should trigger the child question:
              </div>
              <Row>
                <Col md={12}>
                  {responses.map((response, index) => (
                    <Form.Check
                      ...
                      checked={checkedBoxes[response.responseNumber]}
                      onChange={() => {
      ...
      <Modal.Footer>
        <Button onClick={() => close()} id="resetButton">
          Cancel
        </Button>
        <Button onClick={() => save()} id="saveButton">
          Confirm Changes
        </Button>
      </Modal.Footer>
    </Modal>
