...
async function applyFilters(questions, filters) {
  ...
  if (filters.roles) {
    ...
    for (let dim of Object.keys(questions).filter((k) => String(k) !== '1')) {
      questions[dim] = questions[dim].filter((q) =>
        filters.roles.some((role) => q.roles?.includes(role))
      );
    }
  }
    ...
    for (let dim of Object.keys(questions).filter((k) => String(k) !== '1')) {
      questions[dim] = questions[dim].filter((q) =>
        filters.regions.some((region) =>
          q.regionalApplicability?.includes(region)
        )
      );
    }
  }
  ...
  if (filters.lifecycles) {
    ...
    for (let dim of Object.keys(questions).filter((k) => String(k) !== '1')) {
      questions[dim] = questions[dim].filter((q) =>
        filters.lifecycles.some((lifecycle) => q.lifecycle?.includes(lifecycle))
      );
    }
  }
    ...
    for (let dim of Object.keys(questions).filter((k) => String(k) !== '1')) {
      questions[dim] = questions[dim].filter((q) =>
        filters.domains.some((domain) =>
          q.domainApplicability?.includes(domain)
        )
      );
    }
  }
...
async function createPages(q, filters) {
  ...
  for (let dimension of Object.keys(dimQuestions)) {
    ...
    for (let question of dimQuestions[dimension]) {
      ...
      if (questions.length == 4) {
        var dimPage = createPage(
          questions,
          Dimensions[question.trustIndexDimension].page + pageCount,
          Dimensions[question.trustIndexDimension].name,
          Dimensions,
          Children
        );
    ...
    if (questions.length > 0) {
      var dimPage = createPage(
        questions,
        Dimensions[questions[0].trustIndexDimension].page + pageCount,
        Dimensions[questions[0].trustIndexDimension].name,
        Dimensions,
        Children
      );
      page.pages.push(dimPage);
...
router.get('/:questionId', async (req, res) => {
  ...
  Question.findOne({ _id: req.params.questionId })
    .then((question) =>
      res.status(200).send(formatQuestion(question, Dimensions))
    )
    .catch((err) => res.status(400).send(err));
});
